...
SS  = cheb_fet.FETMixedFunctionSpace([S, S])
...
sw = Function(SS)
(s_t, w) = cheb_fet.FETsplit(sw)

# Test functions (Space-time)
v_sw = TestFunction(SS)
(v_w, v_s) = cheb_fet.FETsplit(v_sw)  # Switching the order to make the assembled matrix more symmetric
...
F = cheb_fet.residual(
    F,
    lambda a, b, c : a * (grad(b)[0] * grad(c)[1] - grad(b)[1] * grad(c)[0]) * dx,
    (w, s_tilde, v_s)
)
# F = cheb_fet.residual(
#     F,
...
F = cheb_fet.residual(
    F,
    lambda a, b : - Hdiv0(grad(a), grad(b)),
    (s, v_w)
)
...
Write text outputs
'''
# Compute auxiliary vorticity
def compute_auxiliary_enstrophy(s_ae):
    # Functions
    w_ae = Function(S_)
    v_ae = TestFunction(S_)

    # Auxiliary vorticity residual
    F = (
        Hdiv0(grad(s_ae), grad(v_ae))
      - inner(grad(w_ae), grad(v_ae)) * dx
    )

    # Solve
    solve(F==0, w_ae, bcs=[DirichletBC(S_, 0, "on_boundary")])

    # Output
    return assemble(1/2 * w_ae**2 * dx)
...
def print_write_qoi(qoi_name, qoi_file, qoi_operator, write_type):
    ...
    if mesh.comm.rank == 0:
        open("output/10_feec/avfet/" + qoi_file + ".txt", write_type).write(str(qoi) + "\n")
...
for (i, Re_) in enumerate(Re_arr):
    ...
    '''
    # Set up Paraview
    pvd = VTKFile("output/10_feec/avfet/solution.pvd")
    ...
    while (time < duration - float(timestep)/2):
        ...
        s_.assign(cheb_fet.FETeval((s_, None), (sw, 0), timestep, timestep))
