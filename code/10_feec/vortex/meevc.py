...
save_int = 16  # How regularly to save output
...
(v_s, v_w) = cheb_fet.FETsplit(v_sw)
...
def print_write_qoi(qoi_name, qoi_file, qoi_operator, write_type):
    ...
    if mesh.comm.rank == 0:
        open("output/10_feec/meevc/" + qoi_file + ".txt", write_type).write(str(qoi) + "\n")
...
for (i, Re_) in enumerate(Re_arr):
    ...
    '''
    # Set up Paraview
    pvd = VTKFile("output/10_feec/meevc/solution.pvd")
