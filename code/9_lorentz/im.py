...
# Output
folder = terminal_options.get("folder", type=str, default="9_lorentz/im")
...
S_temp = VectorFunctionSpace(mesh, "CG", 1, dim=3)

# Functions
z = Function(Z, name="Solution")
(x, v) = split(z)
(y, w) = split(TestFunction(Z))
...
F = (
    (  # Position
        inner(x.dx(0), y.dx(0)) * dx_h
      - inner(v, y.dx(0)) * dx_h
    )
  + (  # Velocity
        inner(v.dx(0), w.dx(0)) * dx_h
      - 1/rho * inner(cross(v, B), w.dx(0)) * dx_h
    )
)
...
Initial guesses
'''
z.subfunctions[0].interpolate(x_)
